...
declare module 'react-calendar-timeline' {
    ...
    // @types/react-calendar-timeline v0.28.0 の型定義ファイルに DraggableMarker の定義を追加
    export interface DraggableMarkerChildrenProps {
        styles: React.CSSProperties;
        date: number;
        isDragging: boolean;
    }
    export interface DraggableMarkerProps {
        date: Date | number;
        onDragging?(date: number): void;
        onDragStart?(date: number): void;
        onDragEnd?(date: number): void;
        children?: ((props: DraggableMarkerChildrenProps) => React.ReactNode) | undefined;
    }

    export class DraggableMarker extends React.Component<DraggableMarkerProps> {}
